...
* @Last Modified time: 2014-11-16 05:03:32
*/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
public:
	...
	void init_DQT(int quality)
	{
		quality = max(1, min(99, quality));
		for (int i = 0; i < 64; i++)
		{
			DQT_Y[ZigZag[i]] = max(0, min(0xFF, ((int)Luminance_Quantization_Table[i] * quality + 50) / 100));
			DQT_CbCr[ZigZag[i]] = max(0, min(0xFF, ((int)Chromiance_Quantization_Table[i] * quality + 50) / 100));
		}
	}
	void read_bmp_file(const char *path)
	{
		...
		//BMP使用标准坐标系,左下角为(0,0) 要转换成左上角为(0,0)的形式
	...
	void write_jpeg_header(FILE *file, int height, int width)
	{
		...
		write_8(file, 0x00);			//8bit table 0
		write_block(file, DQT_Y, 64);	//DQT_Y
		write_8(file,0x01);				//8bit table 1
		write_block(file, DQT_CbCr, 64);//DQT_CbCr

		//SOF0, Start of Frame
		write_16(file,0xFFC0);		//标记代码=0xFFC0
		write_16(file,)				//数据长度
		write_8(file,8);			//精度 8bit
		write_16(file,height);		//图像高度
		write_16(file,width);		//图像宽度
		write_8(file,3);			//颜色分量数

		write_8(file,0);			//颜色分量ID
		write_8(file,0x11);			// 水平/垂直采样因子
		write_8(file,0);			//当前分量使用的量化表的ID

		write_8(file,1);			//颜色分量ID
		write_8(file,0x11);			// 水平/垂直采样因子
		write_8(file,1);			//当前分量使用的量化表的ID

		write_8(file,2);			//颜色分量ID
		write_8(file,0x11);			// 水平/垂直采样因子
		write_8(file,1);			//当前分量使用的量化表的ID
